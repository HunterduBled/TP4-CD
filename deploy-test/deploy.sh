...
if [ ! -d "terraform" ] || [ ! -f "terraform/variables.tf" ] || [ ! -f "terraform/main.tf" ] || [ ! -f "terraform/vpc/variables.tf" ] || [ ! -f "terraform/vpc/main.tf" ] || [ ! -f "terraform/vm-test/variables.tf" ] || [ ! -f "terraform/vm-test/main.tf" ] || [ ! -f "terraform/service_account/variables.tf" ] || [ ! -f "terraform/service_account/main.tf" ] || [ ! -f "terraform/firewall/variables.tf" ] || [ ! -f "terraform/firewall/main.tf" ]; then
    echo -e "\033[33mCertains fichiers Terraform sont manquants. Clonage du référentiel...\033[0m"
    git clone https://github.com/HunterduBled/TP4-CD
    cd TP4-CD/deploy-test/terraform
else
    cd terraform
fi
...
sed -i "" "s/^VmTest_internal_ip:.*/VmTest_internal_ip: \"$VmTest_internal_ip\"/" ../ansible/vars.yml
...
if [ ! -d "ansible" ] || [ ! -f "ansible/roles/vm-test/tasks/main.yml" ] || [ ! -f "ansible/vars.yml" ] || [ ! -f "ansible/inventory.ini" ] || [ ! -f "ansible/ansible.cfg" ] || [ ! -f "ansible/playbook.yml" ]; then
    echo "Certains fichiers Ansible sont manquants. Clonage du référentiel..."
    git clone https://github.com/HunterduBled/TP4-CD.git
    cd TP4-CD/deploy-test/ansible
else
    cd ansible
fi
